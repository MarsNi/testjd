...
let tuanActiveId = `u9pm44YKSBNDQTgn7sOCEg==`;
...
async function jdDreamFactory() {
  try {
    ...
    await showMsg();
    if (helpAuthor === true) await joinLeaderTuan();
  } catch (e) {
    $.logErr(e)
  }
}
...
function updateTuanIdsCDN(url = 'https://raw.githubusercontent.com/inoyna11/Write-files/master/shareCodes/jd_updateFactoryTuanId.json') {
  return new Promise(async resolve => {
    $.get({url,
      timeout: 200000,
      headers:{
...
function requireConfig() {
  return new Promise(async resolve => {
    // await updateTuanIdsCDN('https://raw.githubusercontent.com/inoyna11/Write-files/master/shareCodes/jd_updateFactoryTuanId.json');
